...
object Monads extends App {
  ...
  /*
    ...
    1 - implement a Lazy[T] monad =
      computation which will only be executed when its needed
  ...
  val flatMappedInstance = lazyInstance
    .flatMap(x => Lazy(10 * x))

  val flatMappedInstance2 = lazyInstance
    .flatMap(x => Lazy(10 * x))
  ...
  /*
    ...
    Lazy(v).flatMap(f).flatMa(g) = f(v).flatMap(g)
